...
import blurWgsl from "./blur.generated";
import { BLUE_NOISE_SIZE, blueNoiseBytes } from "./blue-noise-128";
import compositeWgsl from "./composite.generated";
import logoWgsl from "./logo.generated";
import rimWgsl from "./rim.generated";
...
const LOGO_ASPECT = 1859 / 407;
const LOGO_HEIGHT_RATIO = 0.32;
const LOGO_MAX_WIDTH_RATIO = 0.86;
...
export function logoPixelSize(
  ...
  let height = size * LOGO_HEIGHT_RATIO;
  let width = height * LOGO_ASPECT;
  const maxWidth = canvasWidth * LOGO_MAX_WIDTH_RATIO;
  if (width > maxWidth) {
    width = maxWidth;
...
export function centeredPlacement(
  ...
  const [logoWidth, logoHeight] = logoPixelSize(reference, width);
